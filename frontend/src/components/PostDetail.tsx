          ...
          <Card className="mb-6">
            <CardHeader className="pb-4">
              <div className="flex-1">
                {/* [수정] 타입 가드를 사용하여 Post와 Question을 구분 */}
                {'category' in item && (
                  <Badge variant="secondary" className="mb-3">{item.category}</Badge>
                )}
                {'status' in item && (
                  <Badge variant={"outline"} className={`mb-3 ${item.status === "답변완료" ? "border-green-600 text-green-600" : ""}`}>
                    {item.status}
                  </Badge>
                )}
                {/* [수정] post -> item 으로 변경 */}
            ...
            <CardContent>
              {/* [수정] post -> item 으로 변경 */}
              <div className="prose max-w-none mb-6 min-h-[100px] whitespace-pre-wrap">
                {item.content}
              </div>
              ...
              <div className="flex items-center gap-4">
                {/* [수정] 타입 가드를 사용하여 Post일 때만 좋아요 버튼 표시 */}
                {'likes' in item && (
                  <Button
