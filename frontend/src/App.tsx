...
export default function App() {
  ...
  const handleSignupAttempt = async (userData: { email: string; name: string; phone: string; password: string; }): Promise<boolean> => {
  ...
  const handleItemClick = (item: any) => {
    if (currentBoard === "info") {
      setPosts((prev) => prev.map((p) => p.id === item.id ? { ...p, views: p.views + 1 } : p));
      setSelectedPost(item);
    } else if (currentBoard === "qna") {
      setQuestions((prev) => prev.map((q) => q.id === item.id ? { ...q, views: q.views + 1 } : q));
      setSelectedQuestion(item);
    } else if (currentBoard === "notice") {
      setNotices((prev) => prev.map((n) => n.id === item.id ? { ...n, views: n.views + 1 } : n));
  ...
  if (selectedPost) {
    ...
    return <PostDetail post={updatedPost} onBack={() => setSelectedPost(null)} canEdit={canEditItem(currentUser, updatedPost.author)} canDelete={canDeleteItem(currentUser, updatedPost.author)} onEdit={() => handleEditClick(updatedPost)} onDelete={() => handleDeletePost(updatedPost.id)} />;
  }

  if (selectedQuestion) {
    const updatedQuestion = questions.find((q) => q.id === selectedQuestion.id) || selectedQuestion;
    return <QuestionDetail question={updatedQuestion} onBack={() => setSelectedQuestion(null)} canEdit={canEditItem(currentUser, updatedQuestion.author)} canDelete={canDeleteItem(currentUser, updatedQuestion.author)} onEdit={() => handleEditClick(updatedQuestion)} onDelete={() => handleDeleteQuestion(updatedQuestion.id)} />;
  }

  if (selectedNotice) {
    const updatedNotice = notices.find((n) => n.id === selectedNotice.id) || selectedNotice;
    return <NoticeDetail notice={updatedNotice} onBack={() => setSelectedNotice(null)} canEdit={canEditItem(currentUser, updatedNotice.author)} canDelete={canDeleteItem(currentUser, updatedNotice.author)} onEdit={() => handleEditClick(updatedNotice)} onDelete={() => handleDeleteNotice(updatedNotice.id)} />;
  }
  ...
  const renderContent = () => {
    ...
    if (currentMenu === "management") {
      if (currentManagement === "cats") {
        return <CatManagement cats={cats} onAddCat={() => setShowAddCatForm(true)} onEditCat={handleEditCat} onDeleteCat={handleDeleteCat} />;
      } else if (currentManagement === "devices") {
        return <DeviceManagement devices={devices} onAddDevice={() => setShowAddDeviceForm(true)} onEditDevice={handleEditDevice} onDeleteDevice={handleDeleteDevice} />;
      }
    }
    if (currentMenu === "home-cam") return <HomeCam />;
                ...
                {viewMode === "card" ? (
                  <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
                    {filteredData.map((item) => {
                      if (currentBoard === "info") return <PostCard key={item.id} post={item as Post} onClick={() => handleItemClick(item)} canEdit={canEditItem(currentUser, item.author)} canDelete={canDeleteItem(currentUser, item.author)} onEdit={() => handleEditClick(item)} onDelete={() => handleDeletePost(item.id)} />;
                      if (currentBoard === "qna") return <QuestionCard key={item.id} question={item as Question} onClick={() => handleItemClick(item)} canEdit={canEditItem(currentUser, item.author)} canDelete={canDeleteItem(currentUser, item.author)} onEdit={() => handleEditClick(item)} onDelete={() => handleDeleteQuestion(item.id)} />;
        ...
        <div className="p-6 border-b border-border">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-2">
              <Button variant="ghost" size="sm" className="p-2"><Bell className="w-4 h-4" /></Button>
              <Button variant="ghost" size="sm" className="p-2" onClick={handleLogout}><LogOut className="w-4 h-4" /></Button>
            </div>
          </div>
          <div className="flex items-center gap-3">
            <Avatar className="w-8 h-8">
              <ImageWithFallback src={currentUser?.profileImage || "https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=32&h=32&fit=crop&crop=face"} alt="사용자 프로필" className="w-full h-full object-cover" />
              <AvatarFallback className="bg-primary text-primary-foreground text-xs">{currentUser?.name?.charAt(0) || "U"}</AvatarFallback>
            </Avatar>
          ...
          <p className="text-sm text-muted-foreground mt-2">고양이 집사들의 공간</p>
        </div>
        <div className="flex-1 p-4">
          <nav className="space-y-2">
            <button onClick={() => handleMenuChange("dashboard")} className={`w-full flex items-center gap-3 px-3 py-2 rounded-md transition-colors ${currentMenu === "dashboard" ? "bg-primary text-primary-foreground" : "hover:bg-gray-100 text-foreground"}`}><LayoutDashboard className="w-4 h-4" /><span>대시보드</span></button>
            <div>
              <button onClick={() => { setIsManagementMenuOpen(!isManagementMenuOpen); if (!isManagementMenuOpen) { handleMenuChange("management"); } }} className={`w-full flex items-center justify-between px-3 py-2 rounded-md transition-colors ${currentMenu === "management" ? "bg-primary text-primary-foreground" : "hover:bg-gray-100 text-foreground"}`}>
                <span className="flex items-center gap-3"><Settings className="w-4 h-4" /><span>관리</span></span>
                {isManagementMenuOpen ? <ChevronDown className="w-4 h-4 text-muted-foreground" /> : <ChevronRight className="w-4 h-4 text-muted-foreground" />}
              </button>
              {isManagementMenuOpen && (
                <div className="ml-7 mt-1 space-y-1">
                  <button onClick={() => handleManagementChange("cats")} className={`w-full text-left px-3 py-2 rounded-md transition-colors ${currentMenu === "management" && currentManagement === "cats" ? "bg-primary text-primary-foreground" : "hover:bg-gray-100 text-foreground"}`}><div className="flex items-center gap-3"><Heart className="w-4 h-4" /><span>고양이</span></div></button>
                  <button onClick={() => handleManagementChange("devices")} className={`w-full text-left px-3 py-2 rounded-md transition-colors ${currentMenu === "management" && currentManagement === "devices" ? "bg-primary text-primary-foreground" : "hover:bg-gray-100 text-foreground"}`}><div className="flex items-center gap-3"><Smartphone className="w-4 h-4" /><span>장치</span></div></button>
                </div>
              )}
            </div>
            <div>
              <button onClick={() => { setIsBoardMenuOpen(!isBoardMenuOpen); if (!isBoardMenuOpen) { handleMenuChange("board"); } }} className={`w-full flex items-center justify-between px-3 py-2 rounded-md transition-colors ${currentMenu === "board" ? "bg-primary text-primary-foreground" : "hover:bg-gray-100 text-foreground"}`}>
                <span className="flex items-center gap-3"><MessageSquare className="w-4 h-4" /><span>게시판</span></span>
                {isBoardMenuOpen ? <ChevronDown className="w-4 h-4 text-muted-foreground" /> : <ChevronRight className="w-4 h-4 text-muted-foreground" />}
              </button>
              {isBoardMenuOpen && (
                <div className="ml-7 mt-1 space-y-1">
                  <button onClick={() => { handleMenuChange("board"); handleBoardChange("info"); }} className={`w-full text-left px-3 py-2 rounded-md transition-colors ${currentMenu === "board" && currentBoard === "info" ? "bg-primary text-primary-foreground" : "hover:bg-gray-100 text-foreground"}`}>정보게시판</button>
                  <button onClick={() => { handleMenuChange("board"); handleBoardChange("qna"); }} className={`w-full text-left px-3 py-2 rounded-md transition-colors ${currentMenu === "board" && currentBoard === "qna" ? "bg-primary text-primary-foreground" : "hover:bg-gray-100 text-foreground"}`}>Q&A</button>
                  <button onClick={() => { handleMenuChange("board"); handleBoardChange("notice"); }} className={`w-full text-left px-3 py-2 rounded-md transition-colors ${currentMenu === "board" && currentBoard === "notice" ? "bg-primary text-primary-foreground" : "hover:bg-gray-100 text-foreground"}`}>공지사항</button>
                </div>
              )}
            </div>
            <button onClick={() => handleMenuChange("home-cam")} className={`w-full flex items-center gap-3 px-3 py-2 rounded-md transition-colors ${currentMenu === "home-cam" ? "bg-primary text-primary-foreground" : "hover:bg-gray-100 text-foreground"}`}><Camera className="w-4 h-4" /><span>홈캠</span></button>
            <button onClick={() => handleMenuChange("schedule")} className={`w-full flex items-center gap-3 px-3 py-2 rounded-md transition-colors ${currentMenu === "schedule" ? "bg-primary text-primary-foreground" : "hover:bg-gray-100 text-foreground"}`}><CalendarIcon className="w-4 h-4" /><span>일정</span></button>
            <button onClick={() => handleMenuChange("statistics")} className={`w-full flex items-center gap-3 px-3 py-2 rounded-md transition-colors ${currentMenu === "statistics" ? "bg-primary text-primary-foreground" : "hover:bg-gray-100 text-foreground"}`}><BarChart3 className="w-4 h-4" /><span>통계조회</span></button>
            <button onClick={() => handleMenuChange("my-page")} className={`w-full flex items-center gap-3 px-3 py-2 rounded-md transition-colors ${currentMenu === "my-page" ? "bg-primary text-primary-foreground" : "hover:bg-gray-100 text-foreground"}`}><User className="w-4 h-4" /><span>마이페이지</span></button>
            {currentUser?.role === "admin" && (<button onClick={() => handleMenuChange("admin")} className={`w-full flex items-center gap-3 px-3 py-2 rounded-md transition-colors ${currentMenu === "admin" ? "bg-primary text-primary-foreground" : "hover:bg-gray-100 text-foreground"}`}><Shield className="w-4 h-4" /><span>관리자</span></button>)}
          </nav>
        </div>
      </div>
